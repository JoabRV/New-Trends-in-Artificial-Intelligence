...
instrucciones = ["calcular IMC", "calcular índice de masa corporal", "IMC", "índice de masa corporal", "masa corporal"]
def speak(text):
 engine = pyttsx3.init()
 engine.say(text)
 engine.runAndWait()
...
def nutriologo_virtual():
 ...
 while True:
  ...
  if "salir" in texto:
   speak("¡Hasta pronto!")
   break
  elif any(instruccion in texto for instruccion in instrucciones):
    ...
    if imc < 18.5:
     speak("Tu IMC es de {:.1f}, lo que indica que tienes bajo peso. Te recomiendo que consultes con un especialista para recibir la atenciónadecuada.".format(imc))
